...
public class BillingMethods1 extends TestBase {
	...
	public void go2BillingSubscriptionAdmin(String firstName, String lastName, String consID, String userName) throws  Exception{
		logInfo("inside go2BillingSubscriptionAdmin() method.");
		System.out.println("inside go2BillingSubscriptionAdmin() method.");
		um.go2Users();
		um.searchUserFromAdvancedSearch(firstName,lastName,consID);
	...
	public boolean verifyCreditCardInformation() throws  Exception{
		...
		Thread.sleep(10000);
		driver().switchTo().frame("hppFrame");  
		waitOnSpinner();
		...
		waitOnSpinner();
		driver().switchTo().frame("hppFrame"); 
		WebElement ccName = driver().findElement(By.xpath("//*[@id='ValidationMessage']"));
		if(ccName.getText().trim().contains("Please specify a valid value")){
			isValidated = true;
			inputText("cssSelector",inputCCName,"Icentris");
			driver().switchTo().defaultContent();
			clickOnElement("xpath",btnSubmitCC);
			
			waitOnSpinner();
			driver().switchTo().frame("hppFrame");  
			WebElement ccNumber = driver().findElement(By.xpath("//*[@id='ValidationMessage']"));
			if(ccNumber.getText().trim().contains("The card number you specified is missing or invalid")){
				isValidated = true;
				...
				waitOnSpinner();
				driver().switchTo().frame("hppFrame");  
				WebElement expDate = driver().findElement(By.xpath("//*[@id='ValidationMessage']"));
				if(expDate.getText().trim().contains("is an expired date")){
					isValidated = true;
					...
					waitOnSpinner();
					driver().switchTo().frame("hppFrame"); 
					WebElement cvv = driver().findElement(By.xpath("//*[@id='ValidationMessage']"));
					if(cvv.getText().trim().contains("Please specify a valid numeric value")){
						isValidated = true;
						...
						waitOnSpinner();
						driver().switchTo().frame("hppFrame"); 
						WebElement address = driver().findElement(By.xpath("//*[@id='ValidationMessage']"));
						if(address.getText().trim().contains("Please specify a valid value")){
							isValidated = true;
							inputText("cssSelector",inputAddress1,"Wood Cross");
							driver().switchTo().defaultContent();
							clickOnElement("xpath",btnSubmitCC);
							
							waitOnSpinner();
							driver().switchTo().frame("hppFrame"); 
							WebElement city = driver().findElement(By.xpath("//*[@id='ValidationMessage']"));
							if(city.getText().trim().contains("Please specify a valid value")){
								isValidated = true;
								inputText("cssSelector",inputCity,"Salt Lake City");
								driver().switchTo().defaultContent();
								clickOnElement("xpath",btnSubmitCC);	
								
								waitOnSpinner();
								driver().switchTo().frame("hppFrame"); 
								WebElement zip = driver().findElement(By.xpath("//*[@id='ValidationMessage']"));
								if(zip.getText().trim().contains("Please specify a valid value")){
									isValidated = true;
									inputText("cssSelector",inputPostalCode,"12345");
									driver().switchTo().defaultContent();
									clickOnElement("xpath",btnSubmitCC);	
								}
								else{
									isValidated = false;
									Assert.assertTrue(isValidated,"Unable to validate the zip code.");
								}
							}
							else{
								isValidated = false;
							...
							isValidated = false;
							Assert.assertTrue(isValidated,"Unable to validate the address.");
						}
					}
					else{
						isValidated = false;
					...
					isValidated = false;
					Assert.assertTrue(isValidated,"Unable to validate the expiry date.");
				}
			}
			else{
				isValidated = false;
	...
	public boolean verifyCVVByCardType(String cardType) throws  Exception{
		...
		waitOnSpinner();
		driver().switchTo().frame("hppFrame");  
		waitOnElement("xpath","//*[@id='ValidationMessage']");
		WebElement expDate = driver().findElement(By.xpath("//*[@id='ValidationMessage']"));
		if(expDate.getText().trim().contains("The CVV must be either 3 or 4 digits")){
			isValidated = true;
		}
		else{
			isValidated = false;
...
//		 Thread.sleep(5000);
	 ...
	 public boolean verifyLoginAlert() throws  Exception{
		 logInfo("inside verifyLoginAlert() method");
		 System.out.println("inside verifyLoginAlert() method");
		 boolean isFound = false;
		 waitOnElement("xpath","//*[@id='users']/table/tbody/tr/td[1]/span");
		 WebElement el = driver().findElement(By.xpath("//*[@id='users']/table/tbody/tr/td[1]/span"));
		 if(el.getText().contains("not allowed for this user")){
			 isFound = true;
		 }
		 return isFound;
	 }
